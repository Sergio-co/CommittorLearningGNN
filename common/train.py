...
def train_one_epoch(epoch_index, train_loader, model_to_train, optimizer, loss_function, kforce=1.0):
    ...
    for i, (data_t0, data_t1, labels) in enumerate(train_loader):
        ...
        if (i + 1) % report_step == 0:
            ...
            print(f'  batch {i + 1} tloss: {avg_loss:.4f} q0: {q0_val:.4f} loss: {ls:.4f} res: {qt_val:.4f} variance: {qa.item():.4f}')
            running_loss = 0.0
...
def val_one_epoch(epoch_index, val_loader, model_to_train, loss_function, kforce=1.0):
    ...
    with torch.no_grad():
        for data_t0, data_t1, labels in val_loader:
            loss, q0, qt, qa, qs = loss_function(model_to_train, data_t0, data_t1, labels, kforce)
